		...
		if 'custom_meshdata' not in bpy.context.object:
			...
			if bpy.context.window_manager.vertexnormals_enabled:
				if context.mode != "EDIT_MESH":
					...
					row = box.row()
					row.prop(bpy.context.window_manager, 'vn_changeasone', text='Edit All')
					...
					if bpy.context.window_manager.vn_generatemode != 'G_FOLIAGE':
						if not (bpy.context.window_manager.vn_genfoliagecalcinverse):
							row = box.row()
					...
					if bpy.context.window_manager.vn_generatemode == 'UPVECT':
						row = box.row()
						label = row.label("Direction:", 'NONE')
						row = box.row()
						row.column().prop(bpy.context.window_manager, 'vn_directionalgendir', text='')
					
					if bpy.context.window_manager.vn_generatemode == 'POINT':
						row = box.row()
						row.prop(bpy.context.window_manager, 'vn_genfoliagecalcinverse', text='Use Backfaces')
...
def initdefaults():
	...
	bpy.types.WindowManager.vn_directionalgendir = bpy.props.FloatVectorProperty(
		default=(0.0,0.0,1.0),
		subtype='TRANSLATION',
		max=1.0,
		min=-1.0,
	...
	bpy.types.WindowManager.vn_genfoliagecalcinverse = bpy.props.BoolProperty(
		default=False)
